<?php
	...
	=============================*/
	/*
		AUTOR: ING. OSCAR BONILLA RODRÍGUEZ
		FECHA: 2017-07
	*/
	/*=====  End of datos  ======*/
	...
	*/
	class Sucursal {
		...
		public function lista(){
			$consulta = "SELECT * FROM sucursal WHERE id < 82;";
			$data = $this->con->consultaRetorno($consulta);
			return $data;
		}

		public function ver(){
			$consulta = "SELECT * FROM sucursal WHERE id = '{$this->id}';";
			$data = $this->con->consultaRetorno($consulta);
			return $data;
		}
	}
	...
	*/
	class Encuesta {
		...
		public function agregarEncuesta(){
			$consulta = "INSERT INTO encuesta (fecha, id_sucursal) VALUES('{$this->fechaActual}', '{$this->iDsucursal}');";
	...
	*/
	class QuejaMain {
		...
		public function listarSucursalesEncuestadas() {
			$consulta = "select t.id_sucursal, t.sucursal from (select sucursal.id as id_sucursal, sucursal.sucursal, respuesta.respuesta, count(id_respuesta) as tota "
							."from sucursal "
							."inner join encuesta "
							."on sucursal.id = encuesta.id_sucursal "
							...
							."group by sucursal,id_respuesta) as t group by t.sucursal;";
			$data = $this->con->consultaRetorno($consulta);
			return $data;
		}

		public function listarResultadoDeEncuestasPorSucursal($id_sucursal = 0) {
			$consulta = "select t.id_sucursal, t.respuesta, t.total from (select sucursal.id as id_sucursal, sucursal.sucursal, respuesta.respuesta, count(id_respuesta) as total "
							."from sucursal "
							."inner join encuesta "
							."on sucursal.id = encuesta.id_sucursal "
							."inner join responde_encuesta "
							...
							."inner join respuesta "
							."on responde_encuesta.id_respuesta = respuesta.id "
							."group by sucursal,id_respuesta) as t where t.id_sucursal = '{$id_sucursal}';";
			$data = $this->con->consultaRetorno($consulta);
			return $data;
		}
	}
