<?php
	...
	*/
	...
	session_start();
	function setSession($valor) { $_SESSION['logSucursal'] = $valor;}
	function getSession() { return $_SESSION['logSucursal']; }
	function setSessionX($valor, $nombre) { $_SESSION[$nombre] = $valor; }
	function getSessionX($nombre) { return $_SESSION[$nombre]; }
	date_default_timezone_set("America/Mexico_City");
	if(isset($_GET['accion'])){
		$accion = $_GET['accion'];
		if ($accion == 'buscarSucursales') {
			$objSucursal = new Sucursal();
			$datos = $objSucursal->lista();
			$resp = array();
			$listaSucursal = array();
			while ($r = $datos->fetch_assoc()) {
				$listaSucursal[] = array('id'=>$r['id'],'sucursal'=>$r['sucursal']);
			}
			$resp['resp'] = 1;
			$resp['sucursales'] = $listaSucursal;
			echo json_encode($resp);
		}
		if ($accion == 'iniciarEncuesta') {
			$idSucursal = $_GET['idSucursal'];
			$objSucursal = new Sucursal();
			$objSucursal->estPropiedad('id',$idSucursal);
			$datos = $objSucursal->ver();
			$resp = array();
			if($datos->num_rows>0) {
				$resp['resp'] = 1;
				setSession($idSucursal);
				$_SESSION['logSucursal'] = $idSucursal;
		...
		if ($accion == 'agregarEncuesta') {
			...
			$sucursal = $_SESSION['logSucursal'];
			$fechaActual = date("Y-m-d");
			$resp = array();
			$objEncuesta = new Encuesta();
			$objEncuesta->estPropiedad('iDsucursal',$sucursal);
		...
		if($accion == 'bucarSucursalesEncuestadas') {
